...
void main() {
    ...
    if (linearDepth >= 1.0) {
        ...
        float colorMultiplier = linearDepth*-0.3+1.0;//sqrt(linearDepth)*.5+.6;
        colorMultiplier = clamp(colorMultiplier, 0.5, 1.0);
        gl_FragColor = vec4(color.r*colorMultiplier, color.g*colorMultiplier, color.b*colorMultiplier, 1.0);
    }
